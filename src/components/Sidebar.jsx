    ...
    <aside style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          color: '#9E9181', letterSpacing: '0.12em',
          textTransform: 'uppercase',
        }}>
          Node Registry
        </div>
        <div style={{ fontSize: 11, color: '#C4B89E', marginTop: 2 }}>
          {allIds.length} node{allIds.length !== 1 ? 's' : ''} · {roots.length} root{roots.length !== 1 ? 's' : ''}
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '8px 8px' }}>
        {allIds.length === 0 ? (
          <div style={{ padding: '12px 8px', fontSize: 12, color: '#C4B89E', fontStyle: 'italic' }}>
            No nodes yet
          </div>
        ) : (
          allIds.map(id => {
            ...
            return (
              <div
                ...
                onMouseEnter={e => { if (!isCycle) e.currentTarget.style.background = '#EDE8DF'; }}
                onMouseLeave={e => { if (!isCycle) e.currentTarget.style.background = 'transparent'; }}
              >
                <span style={{
                  marginTop: 2, color: isCycle ? '#C4623A' : isRoot ? '#2C2018' : '#9E9181',
                  flexShrink: 0,
                }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                    ...
                    {isRoot && !isCycle && (
                      <span style={{
                        fontSize: 9, color: '#9E9181',
                        background: '#EDE8DF', padding: '0 4px',
                    ...
                    {isCycle && (
                      <span style={{
                        fontSize: 9, color: '#9B4828',
                        background: '#F4C5B4', padding: '0 4px',
                        borderRadius: 3,
                      }}>
                        cycle
                      </span>
                  ...
                  <div style={{
                    fontSize: 11, color: n.condition ? '#6B5E4E' : '#C4B89E',
